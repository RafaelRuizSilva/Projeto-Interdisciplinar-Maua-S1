...
class Atualizar(PlanoDeFundo):
    ...
    def pesquisar_pergunta(self):
        ...
        '''
        id = self.id.get().replace(' ','')
        self.df['ID_PERGUNTA'] = self.df['ID_PERGUNTA'].apply(lambda x: str(x))
        if id in list(self.df['ID_PERGUNTA']):
            self.id_real = self.df[self.df['ID_PERGUNTA'] == id]['ID_TEMPORARIO'].values[0]
            dados = self.df[self.df['ID_PERGUNTA'] == id]
            dados = list(dados.values[0])
            self.pergunta.delete(0,END)
            self.pergunta.insert(0,dados[2])
            self.alternativa_a.delete(0, END)
            self.alternativa_a.insert(0, dados[3])
            self.alternativa_b.delete(0, END)
            self.alternativa_b.insert(0, dados[4])
            self.alternativa_c.delete(0, END)
            self.alternativa_c.insert(0, dados[5])
            self.alternativa_d.delete(0, END)
            self.alternativa_d.insert(0, dados[6])
            self.nivel.delete(0, END)
            self.nivel.insert(0, dados[8])
            self.resp.delete(0, END)
            self.resp.insert(0, dados[7])
